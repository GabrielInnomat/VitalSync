...
- **Kritisch**
- **Status**: gelöst — empirisch **bestätigt und behoben**. Ein Probe-Harness gegen einen echten Wolverine-6.23.0-Host hat den Befund verifiziert: Der per DI in den Transport injizierte `IMessageBus` war ein **frischer `MessageContext` mit `Envelope = null`** (das GH-3001-Scope-Priming griff für diesen Auflösungspfad nicht) — Correlation-Id ging verloren, und bei einem nach dem Publish werfenden Projection-Handler ging das Integration Event **vor** dem Retry raus (Duplikat-Pfad real). Zusätzlicher Fund aus der Verifikation: Unter Wolverines 6er-Default `ServiceLocationPolicy.NotAllowed` schlug bereits die **erste Zustellung** eines Domain Events mit `InvalidServiceLocationException` fehl, weil die internen Handler-Abhängigkeiten Service-Location erzwingen — behoben durch explizite `AlwaysUseServiceLocationFor`-Opt-ins in `ApplyBuildingBlockDomainEventRouting`. Umsetzung wie in Schritt 2/3 vorgeschlagen: `DomainEventEnvelopeHandler` nimmt `IMessageContext` als Wolverine-Handler-Parameter, `IIntegrationEventSink` (Application) macht die Senke in der Signatur von `IDomainEventPublisher.PublishAsync` sichtbar, `WolverineIntegrationEventSink` kapselt den Kontext, `NullIntegrationEventSink` bleibt der Default ohne Messaging; das Transport-Trio (`IIntegrationEventTransport` & Implementierungen) ist ersatzlos entfernt. Regressionstests: `IntegrationEventSinkDeliveryTests` (Correlation-Propagation end-to-end; Integration Event wird bei Handler-Fehlschlag zurückgehalten). Die aus IMP-08 übernommene Flush-on-Commit-Verifikation ist ebenfalls end-to-end umgesetzt (`OutboxFlushOnCommitTests`, Testcontainers): **beide** Persistenzpfade stellen ohne Zutun des Durability-Agents (Polling auf 1 h) sofort nach Commit zu. Dabei fand der EF-Pfad-Test eine **weitere Produktionslücke**: `UseEfCorePersistence` registrierte keinen datenbankgestützten Wolverine-Message-Store — der erste Commit schlug mit „not using Database backed message persistence" fehl. Ursache: Container-registrierte `IWolverineExtension`s werden erst **nach** dem Bau des Providers angewandt, ihre `options.Services.*`-Registrierungen verpuffen. Fix: `EfCoreMessageStoreRegistration` registriert den PostgreSQL-Message-Store zur Kompositionszeit (Wolverines eigene `PostgresqlBackedPersistence.Configure` gegen eine Wegwerf-`WolverineOptions`, deren `Services` auf die echte Collection zeigt); der Options-Anteil (Codegen, Policies, EF-Transaktions-Middleware) bleibt in `ApplyBuildingBlockEfCoreOutbox`. Der e2e-Test pinnt die dabei genutzten Wolverine-Interna gegen Upgrades.
...
**Zusätzlich im selben Test-Harness abzudecken (aus IMP-08 übernommen):** Verifizieren, dass Wolverine die Outbox nach erfolgreichem Commit tatsächlich **sofort** flusht — für **beide** Persistenzpfade:

- **Marten:** `IMartenOutbox.Enroll` registriert den Session-Listener `FlushOutgoingMessagesOnCommit`, dessen `AfterCommitAsync` `FlushOutgoingMessagesAsync()` aufruft (per Quelltext-Inspektion in Wolverine V6.23.0 belegt, aber bei uns nur mit Mocks gepinnt — nicht end-to-end getestet).
- **EF Core:** `SaveChangesAndFlushMessagesAsync` committet und flusht atomar.

Testidee: Command über den echten Host absetzen, danach prüfen, dass der `DomainEventEnvelopeHandler` **ohne** Zutun des Durability-Agents (Polling deaktivieren bzw. Intervall auf Stunden setzen) zeitnah aufgerufen wird. Schlägt der Test nach einem Wolverine-Upgrade fehl, hat sich das Flush-on-Commit-Verhalten geändert — genau davor soll er schützen. **Umgesetzt** in `OutboxFlushOnCommitTests` (Testcontainers, beide Persistenzpfade).
...
- **Kritisch**
-   - **Status**: gelöst. Der Cache-Schlüssel für `CommandWithResultDispatchers` und `QueryDispatchers` ist jetzt ein `readonly record struct DispatcherKey(Type Request, Type Result)` — der Ergebnistyp geht damit in den Schlüssel ein, und ein Typ mit zwei Ergebnisverträgen löst den jeweils richtigen Dispatcher auf (Schritt 1). `CommandDispatchers` (ohne Ergebnistyp) bleibt unverändert. Zusätzlich lehnt der `HandlerRegistrationStartupValidator` (IMP-05) Request-Typen ab, die mehr als einen `ICommand<>`/`IQuery<>`-Vertrag implementieren — der Modellierungsfehler wird zum Startfehler statt nur überlebt (Schritt 2). Der zuvor als `Skip` hinterlegte Regressionstest `Send_SameRequestTypeWithDifferentResultTypes_ResolvesCorrectDispatcher` ist aktiviert; die Startprüfung ist durch `HandlerStartupValidationTests` mit der Fixture-Assembly `AmbiguousRequestsFixture` abgesichert (Schritt 3).
...
- **Kritisch**
-   - **Status**: gelöst. `UnitOfWorkBehavior` bekommt die Unit of Work jetzt als **nullable Konstruktorparameter** (`IUnitOfWork? unitOfWork = null`) direkt injiziert statt sie per `GetRequiredService` aus dem Container zu ziehen; ohne registrierte Persistenz laufen Commands unverändert durch, ohne Commit und ohne Exception (Schritt 1 — löst zugleich IMP-46 Schritt 1: das Behavior ist ohne Container direkt instanziierbar). Fehlt am Ende von `AddBuildingBlocks` eine `IUnitOfWork`-Registrierung, registriert die Methode den `MissingUnitOfWorkStartupLogger` (ein `IHostedService`), der beim Host-Start einmalig als `Information` loggt, dass Commands nicht committet werden — der stille No-Op wird sichtbar (Schritt 2). Der zuvor als `Skip` hinterlegte Test `SuccessfulCommand_WithoutRegisteredUnitOfWork_PassesThrough` ist aktiviert; dazu Tests für die direkte Instanziierung ohne Container, den Startup-Log und dessen Abwesenheit bei registrierter Unit of Work. Die Tests aus Schritt 3 (`FailedCommand_DoesNotCommit`, `Query_DoesNotCommit`) existierten bereits in `UnitOfWorkBehaviorTests`.
...
- **Kritisch**
-   - **Status**: gelöst — als **nicht (mehr) zutreffend verifiziert**. Die Prämisse des Befunds ist mit Wolverine 6 veraltet: `IMartenOutbox` besitzt gar kein `FlushOutgoingMessagesAsync()` (der vorgeschlagene Code kompiliert nicht). Stattdessen registriert `MartenOutbox.Enroll(session)` — das `MartenUnitOfWork` als ersten Schritt von `CommitAsync` aufruft — den Session-Listener `FlushOutgoingMessagesOnCommit`, dessen `AfterCommitAsync` nach jedem erfolgreichen `SaveChangesAsync` automatisch `FlushOutgoingMessagesAsync()` ausführt (verifiziert im Wolverine-Quelltext, `src/Persistence/Wolverine.Marten/`). Der Marten-Pfad flusht also bereits sofort beim Commit — in der korrekten Reihenfolge (erst Commit, dann Flush, von Wolverine selbst garantiert) und symmetrisch zum EF-Pfad (`SaveChangesAndFlushMessagesAsync`); der Durability Agent bleibt reine Crash-Recovery. Die verbleibende Verantwortung unseres Codes — `Enroll` **vor** `SaveChangesAsync` — ist jetzt durch `MartenUnitOfWorkTests` festgeschrieben (dazu: Events werden vor dem Save in die Outbox publiziert, Events werden nach erfolgreichem Commit geräumt bzw. bei fehlgeschlagenem Commit behalten). Schritt 2 (Template-Basisklasse) wurde **bewusst verworfen**: Es gibt genau zwei sanktionierte Persistenzstile (ADR-0019/0020), die Symmetrie ist durch Tests statt durch eine Abstraktion mit nur zwei Nutzern abgesichert; eine Basisklasse wird erst extrahiert, falls je ein dritter Store per ADR beschlossen wird. Das Flush-Verhalten ist inzwischen auch **end-to-end** abgesichert: `OutboxFlushOnCommitTests` (Testcontainers) beweist für beide Persistenzpfade die sofortige Zustellung nach Commit ohne Durability-Agent — siehe IMP-04-Status.
...
- **Niedrig**
-   - **Status**: teilweise gelöst (mit IMP-07). Schritt 1 ist umgesetzt: `UnitOfWorkBehavior` nutzt Konstruktorinjektion mit nullable `IUnitOfWork?` und ist ohne Container testbar (abgesichert durch einen Direktinstanziierungs-Test). Schritt 2 (begründende `<remarks>` in `Sender`/`ProjectionRunner`) und Schritt 3 (Regel festschreiben) stehen noch aus.
