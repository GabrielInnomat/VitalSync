...
`GaWeCodes.Thessera.Core` is the single outer layer of the Thessera
...
`GaWeCodes.Thessera.Domain` and `GaWeCodes.Thessera.Application` and is the **only**
Thessera package allowed to reference third-party packages.
...
- **All third parties live here.** EF Core, Marten, Wolverine, Npgsql, and DI
  ...
  `GaWeCodes.Thessera.Persistence`, `GaWeCodes.Thessera.Persistence.EfCore.Postgres`,
  `GaWeCodes.Thessera.Persistence.Marten` and `GaWeCodes.Thessera.Messaging.RabbitMq`.
  ...
  They share the `GaWeCodes.Thessera.*` namespace root, so a consumer's `using`
...
| --------------------------------- | ----------------------------------------------------------------- |
| `ServiceCollectionExtensions`     | `AddThessera` — the entry point                             |
| `HostApplicationBuilderExtensions`| `AddThessera` on the host builder (ADR-0027)                |
| `ThesseraOptions`           | the configuration surface passed to it                            |
| `EntityKeyModelBuilderExtensions` | `ApplyEntityKeyConversions`, called from a host's `DbContext`     |
| `PersistedSchema`                 | the event-schema snapshot, called from a service's tests (ADR-0035); it lives in `GaWeCodes.Thessera.Testing`, not in the core |
...
| Type                                | Who uses it                                                       |
| ----------------------------------- | ----------------------------------------------------------------- |
| `IPersistenceAdapter`               | implemented by each adapter; chosen via `ThesseraOptions.UsePersistence` |
...
| `IMessagingTransportAdapter`        | implemented by a transport; chosen via `ThesseraOptions.UseMessagingTransport` |
...
`ThesseraOptions.UsePersistence` and `EntityKeyJsonOptions.Create()` are public for the
same reason.
...
`AddThessera`. `ExtensionPointTests` proves each of them end to end using only the
...
- **Configuring Wolverine is not messaging.** `ThesseraWiringSettings`, its three
  ...
  `ThesseraWolverineExtension` describe what the host asked for and translate it
...
- **The settings type is an aggregate, and consumers do not take it.**
  `ThesseraWiringSettings` holds nothing but `Persistence`, `Messaging`,
  ...
  (`RequiresWolverine`, `ThesseraWolverineExtension`, `WolverineRuntimeCheck`) take
...
Wolverine via `UseSystemTextJsonForSerialization` in `ThesseraWolverineExtension`. Its
...
`GaWeCodes.Thessera.Testing`, so that a runtime host never carries an approval-test tool — renders every
...
Implements the `ISender` contract from `GaWeCodes.Thessera.Application`
(ADR-0015):
...
- **Behavior ordering is an explicit numeric order** — each behavior is registered
  ...
  position via `ThesseraOptions.AddPipelineBehavior(type, order)` — that is the
  **only** supported way; a behavior registered directly on the `IServiceCollection`
  has no order and fails `AddThessera` (see §Composition root).
...
| `UnitOfWorkBehavior`        | `300` (innermost) | Commits the unit of work when a **command** completes successfully — including the atomic outbox write (see §2/§4). Queries and failed results are unaffected: nothing is committed, and query-only hosts need no `IUnitOfWork` registration. The dependency is **non-optional**: Thessera registers a `NullUnitOfWork` fallback so the pipeline always resolves, and `UnitOfWorkPresenceCheck` fails the host at start when that fallback would silently swallow real commands (see §Composition root). A host that deliberately commits nothing says so with `UseNoPersistence()`. Translates the store's optimistic-concurrency exceptions raised on commit (Marten's `ConcurrencyException`, EF Core's `DbUpdateConcurrencyException`) into a `Failure` with category `Conflict`, and does the same for a PostgreSQL unique-constraint violation (SQLSTATE `23505`) — expected on both persistence paths, so it is caught both wrapped in a `DbUpdateException` and bare, as Marten raises it. |
...
- Both implementations obtain the empty hull they rehydrate into through the
  ...
  **private parameterless constructor** — no `new()` constraint, no public
  ...
  is validated at host startup: `AddThessera` checks every aggregate in
...
- **Retries are graded by failure class** (ADR-0023 amendment 2026-08-06). One rule for
  ...
  `ApplyThesseraMessagingDefaults` registers three, and Wolverine takes the first
...
- **Routing.** Connecting the transport moves nothing on its own — Wolverine
  ...
  `GaWeCodes.Thessera.Application` — resolved by the routing rule's topic source, so
...
- **Context identity.** The host also names its own bounded context
  ...
  header `thessera.source-context`, and a consumer-side middleware discards an
...
- **Durability.** The topology is declared durable end to end (ADR-0023 amendment
  ...
  declaration, which also covers the queues Thessera never names itself —
...
- **Publisher confirmations close the last metre.** A durable outbox deletes its row
  ...
  *in the socket*, not *in the broker* — a broker that discards the message
  ...
  through unchanged. `ApplyThesseraMessagingDefaults` therefore calls
...
- **The inbox idempotency window is a decision.** The subscriber queue listens with
  ...
  the guarantee silently expired. `ApplyThesseraIdempotencyWindow` widens it to
...
- `UseWolverineMessaging(rabbitMqUri, exchangeName, contextName)` takes the three
  ...
  not to Thessera — VitalSync defines it once in `VitalSync.ServiceDefaults` and
...
- **Startup handler validation always runs**: `AddThessera` registers a
...
- The remaining defaults come from a registered `IWolverineExtension`
  (`ThesseraWolverineExtension`, ADR-0027): domain-event routing whenever
...
- This is the **only** way to obtain the EF Core outbox: the former public
  `UseThesseraEfCorePersistence(cs)` is gone, so no host can point the
...
`ThesseraOptions` is the fluent surface a host sees and nothing more. Each of its
...
The work itself lives in the internal `ThesseraComposition`, which runs six
named phases in a fixed order:

| Phase                    | Does                                                                    |
| ------------------------ | ----------------------------------------------------------------------- |
| `EnsureSingleCall`       | rejects a second `AddThessera` on the same service collection      |
...
### `AddThessera` is called exactly once
...
`PipelineBehaviorRegistry`, the `ThesseraWiringSettings`, and the
...
`AddThesseraCore`, so there is one place to guard. The three shared objects are
...
its own check the same way Thessera registers its own, and the runner picks it up
...
A single `StartupCheckRunner` — the only `IHostedLifecycleService` Thessera
...
- **`UnitOfWorkPresenceCheck` probes the built container**, by resolving `IUnitOfWork`
  ...
  that registers `IUnitOfWork` *after* `AddThessera` used to be flagged wrongly;
  now it is not.
...
The dependency is therefore non-optional. Thessera registers a `NullUnitOfWork`
...
`GaWeCodes.Thessera.Persistence.Marten` a plain `using Marten;` binds to the
...
`[IntegrationEventTopic]` in `GaWeCodes.Thessera.Application` is the attribute it reads. A
...
The four Wolverine wiring extensions are `ApplyThesseraIdempotencyWindow`,
`ApplyThesseraDomainEventRouting`, `ApplyThesseraMessagingDefaults`, and
`ApplyThesseraSubscription` — plural, matching the assembly. **ADR-0023 and ADR-0027
...
`IClock` is the narrow time port declared in `GaWeCodes.Thessera.Domain` ("the
...
**`GaWeCodes.Thessera`** and its version is the assembly's `AssemblyInformationalVersion`; every tag
starts with `thessera.`, matching the existing message header
`thessera.source-context`.
...
- **The name must not say `vitalsync`.** ADR-0018 guarantees the string does not occur anywhere
  under `Thessera/src`. The source name is therefore `GaWeCodes.Thessera`, and the host
  ...
  `GaWeCodes.Thessera.Core`, which has none today, and would drag Marten, Wolverine, and
  ...
  `TracingTests` proves Thessera emits under it.
...
- **HTTP/gRPC status mapping** — the BFF and service hosts own transport
  ...
  [GaWeCodes.Thessera.Application](./thessera-application.md)).
...
`GaWeCodes.Thessera.Tests` mirrors this project. Tests use xUnit
