# GaWeCodes.Thessera.Domain — Technical Reference

`GaWeCodes.Thessera.Domain` is the foundational package of the platform. It provides the tactical Domain-Driven Design primitives that every microservice's domain layer builds upon, and it is deliberately kept pure — no framework, no infrastructure, BCL only.
...
> Scope: this document describes the _domain_ package only. Application, and Infrastructure are documented separately.
...
namespace `GaWeCodes.Thessera.Domain` is deliberately almost empty.
...
```xml
<ItemGroup>
  <Using Include="GaWeCodes.Thessera.Domain.Aggregates" />
  <Using Include="GaWeCodes.Thessera.Domain.Entities" />
  <Using Include="GaWeCodes.Thessera.Domain.Events" />
  <Using Include="GaWeCodes.Thessera.Domain.Naming" />
  <Using Include="GaWeCodes.Thessera.Domain.Rules" />
</ItemGroup>
```
...
per service, rendered by `PersistedSchema` in `GaWeCodes.Thessera.Testing` and asserted by a test (ADR-0035).
