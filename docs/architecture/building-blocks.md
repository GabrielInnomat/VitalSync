...
> Detailed reference: [GaWeCodes.Domain](./building-blocks-domain.md).

### `GaWeCodes.Application`

`GaWeCodes.Application` is the framework-agnostic use-case layer. It defines the **CQRS abstractions** (commands, queries, and their handlers), the **pipeline-behavior** contract, the **dispatcher** (`ISender`) contract, and the **`Result` / `Failure`** model.
...
> Detailed reference: [GaWeCodes.Application](./building-blocks-application.md).

### `GaWeCodes.Composition`

`GaWeCodes.Composition` is the single outer layer that holds **all** the reusable, framework-bound, third-party-backed implementations that are still **independent of any VitalSync domain logic**:
...
> Detailed reference: [GaWeCodes](./building-blocks-infrastructure.md).
...
- **CQRS by default.** Commands and queries are explicit and separated at the Application layer. Contracts and the `Result` model live in `GaWeCodes.Application`; a **hand-rolled dispatcher** implements them in `Infrastructure`. See [ADR-0015](./decisions/0015-hand-rolled-cqrs-mediator.md).
