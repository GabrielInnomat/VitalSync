...
- **A context does not consume its own integration events.** Every published event carries the header `gawecodes.source-context`; a consumer-side middleware discards an integration event whose source is the consuming context itself. This is what a pattern like `nutrition.*` in the Nutrition service used to do — deliver its own events back to it.
