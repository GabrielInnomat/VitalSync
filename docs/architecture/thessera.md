...
**Thessera** is a reusable platform of shared concepts and components that underpins the microservices **without coupling them to VitalSync**. It is designed to be reusable in future projects.
...
- Keep the **Domain** and **Application** packages free of any framework or third-party dependency.
...
> Detailed reference: [GaWeCodes.Thessera.Domain](./thessera-domain.md).

### `GaWeCodes.Thessera.Application`

`GaWeCodes.Thessera.Application` is the framework-agnostic use-case layer. It defines the **CQRS abstractions** (commands, queries, and their handlers), the **pipeline-behavior** contract, the **dispatcher** (`ISender`) contract, and the **`Result` / `Failure`** model.
...
> Detailed reference: [GaWeCodes.Thessera.Application](./thessera-application.md).

### `GaWeCodes.Thessera.Core`

`GaWeCodes.Thessera.Core` is the single outer layer that holds **all** the reusable, framework-bound, third-party-backed implementations that are still **independent of any VitalSync domain logic**:
...
> Detailed reference: [GaWeCodes.Thessera.Core](./thessera-core.md).
...
- **CQRS by default.** Commands and queries are explicit and separated at the Application layer. Contracts and the `Result` model live in `GaWeCodes.Thessera.Application`; a **hand-rolled dispatcher** implements them in `Infrastructure`. See [ADR-0015](./decisions/0015-hand-rolled-cqrs-mediator.md).
- **Uniform failure channel.** Expected domain errors are translated to `Result.Failed` at the Application boundary; unexpected errors are handled globally. See [ADR-0017](./decisions/0017-application-error-handling-and-result.md).
...
Each package has a corresponding test project. See [Testing strategy](./testing-strategy.md).
