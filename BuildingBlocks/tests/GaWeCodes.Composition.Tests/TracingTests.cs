...
{
    private const string ActivitySourceName = "GaWeCodes";
    ...
    public async Task ASucceedingCommand_ProducesOneSpanNamedAfterTheRequest()
    {
        ...
        Assert.Equal("TracedCommand", span.GetTagItem("gawecodes.request.name"));
        Assert.Equal("command", span.GetTagItem("gawecodes.request.kind"));
        Assert.Equal("success", span.GetTagItem("gawecodes.outcome"));
    ...
    public async Task AQuery_IsTaggedAsAQueryNotACommand()
    {
        ...
        Assert.Equal("query", span.GetTagItem("gawecodes.request.kind"));
    }
    ...
    public async Task AnExpectedDomainFailure_IsNotAnErrorSpanButNamesItsCategories()
    {
        ...
        var span = Assert.Single(recorder.Spans);
        Assert.Equal(ActivityStatusCode.Ok, span.Status);
        Assert.Equal("failure", span.GetTagItem("gawecodes.outcome"));
        Assert.Equal(
            FailureCategory.BusinessRule.ToString(),
            span.GetTagItem("gawecodes.failure.categories"));
    }
    ...
    public async Task AnUnexpectedException_MarksTheSpanAsErrorAndNamesTheExceptionType()
    {
        ...
        Assert.Equal("faulted", span.GetTagItem("gawecodes.outcome"));
        Assert.Equal(typeof(InvalidOperationException).FullName, span.GetTagItem("gawecodes.exception.type"));
    }
    ...
    public async Task EachProjectionHandler_GetsItsOwnSpanNamingTheHandlerAndTheAggregate()
    {
        ...
        Assert.Equal(typeof(TracedFirstProjection).FullName, projection.GetTagItem("gawecodes.projection.handler"));
        Assert.Equal("probe", projection.GetTagItem("gawecodes.aggregate.name"));
        Assert.Equal("probe-1", projection.GetTagItem("gawecodes.aggregate.id"));
        Assert.Equal(7L, projection.GetTagItem("gawecodes.aggregate.version"));
    }
    ...
    public async Task AProjectionSpan_IsNotNestedInsideThePublishSpan()
    {
        ...
        Assert.Equal(0, publish.GetTagItem("gawecodes.integration_events.published"));
    }
    ...
    public async Task ThePublishSpan_CountsThePublishedIntegrationEvents()
    {
        ...
        Assert.Equal(2, publish.GetTagItem("gawecodes.integration_events.published"));
    }
