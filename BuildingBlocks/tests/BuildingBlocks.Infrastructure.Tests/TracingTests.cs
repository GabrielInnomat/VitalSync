...
{
    private const string ActivitySourceName = "BuildingBlocks";
    ...
    public async Task ASucceedingCommand_ProducesOneSpanNamedAfterTheRequest()
    {
        ...
        Assert.Equal("TracedCommand", span.GetTagItem("buildingblocks.request.name"));
        Assert.Equal("command", span.GetTagItem("buildingblocks.request.kind"));
        Assert.Equal("success", span.GetTagItem("buildingblocks.outcome"));
    ...
    public async Task AQuery_IsTaggedAsAQueryNotACommand()
    {
        ...
        Assert.Equal("query", span.GetTagItem("buildingblocks.request.kind"));
    }
    ...
    public async Task AnExpectedDomainFailure_IsNotAnErrorSpanButNamesItsCategories()
    {
        ...
        var span = Assert.Single(recorder.Spans);
        Assert.Equal(ActivityStatusCode.Ok, span.Status);
        Assert.Equal("failure", span.GetTagItem("buildingblocks.outcome"));
        Assert.Equal(
            FailureCategory.BusinessRule.ToString(),
            span.GetTagItem("buildingblocks.failure.categories"));
    }
    ...
    public async Task AnUnexpectedException_MarksTheSpanAsErrorAndNamesTheExceptionType()
    {
        ...
        Assert.Equal("faulted", span.GetTagItem("buildingblocks.outcome"));
        Assert.Equal(typeof(InvalidOperationException).FullName, span.GetTagItem("buildingblocks.exception.type"));
    }
    ...
    public async Task EachProjectionHandler_GetsItsOwnSpanNamingTheHandlerAndTheAggregate()
    {
        ...
        Assert.Equal(typeof(TracedFirstProjection).FullName, projection.GetTagItem("buildingblocks.projection.handler"));
        Assert.Equal("probe", projection.GetTagItem("buildingblocks.aggregate.name"));
        Assert.Equal("probe-1", projection.GetTagItem("buildingblocks.aggregate.id"));
        Assert.Equal(7L, projection.GetTagItem("buildingblocks.aggregate.version"));
    }
    ...
    public async Task ThePublishSpan_CountsThePublishedIntegrationEvents()
    {
        ...
        Assert.Equal(2, publish.GetTagItem("buildingblocks.integration_events.published"));
    }
