...
public sealed class DomainEventPartitioningBehaviourTests(PostgreSqlFixture postgres)
{
    ...
    [Fact]
    public async Task TheBuildingBlocksQueues_ArePartitionedRatherThanGloballySequential()
    {
        Assert.SkipUnless(postgres.Available, postgres.SkipReason);
        ...
        foreach (var queueName in new[] { "building-blocks-domain-events", "building-blocks-projections" })
        {
            ...
            Assert.Equal<PartitionSlots?>(BuildingBlocksWiring.DomainEventPartitionSlots, endpoint.GroupShardingSlotNumber);
